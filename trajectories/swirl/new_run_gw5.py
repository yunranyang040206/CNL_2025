...
DATASET = sys.argv[2] if len(sys.argv) > 2 else "gw5_0"
...
EMBED_DIR = os.path.join(BASE_DIR, "output", "gtrxl")
...
def build_true_behavior_oracle_gw5(trans_prob, grid=5, water_state=24,
    ...
    def boundary_dist(s):
        x, y = to_xy(s)
        return min(x, grid - 1 - x, y, grid - 1 - y)

    def is_perimeter(s):
        x, y = to_xy(s)
        return x == 0 or x == grid - 1 or y == 0 or y == grid - 1
    ...
    for s in range(S):
        ...
        # mode 1: exact choose_water_action distribution
        best = []
        ...
        p_after_hesitate = 1.0 - water_hesitate
        pi[1, s, :] += p_after_hesitate * water_random / A
        p_greedy = p_after_hesitate * (1.0 - water_random)
        if best:
            pi[1, s, best] += p_greedy / len(best)
        else:
            pi[1, s, 4] += p_greedy

        # mode 0: exact choose_explore_action distribution
        valid = []
        for a in range(A):
            probs = trans_prob[s, a]
            ns = int(np.argmax(probs))
            if ns != water_state:
                valid.append(a)
        ...
        curr_bd = boundary_dist(s)
        curr_on_perim = is_perimeter(s)
        scores = []
        for a in valid:
            probs = trans_prob[s, a]
            ns = int(np.argmax(probs))
            ...
            next_bd = boundary_dist(ns)
            if next_bd < curr_bd:
                score += 1.5
            elif next_bd == curr_bd:
                score += 0.25

            if curr_on_perim and is_perimeter(ns):
                score += 1.0
        ...
        best_idx = np.flatnonzero(scores == scores.max())
        best_actions = [valid[i] for i in best_idx]
        pi[0, s, best_actions] += (1.0 - explore_random) / len(best_actions)
...
def build_logemit_list(R_state, hoh, trans_probs_j, discount=0.95, vi_iters=50, tau=1.0,
    ...
    and the SAME planning dynamics everywhere.
    """
    eps = 1e-20

    tp_plan = make_goal_absorbing(trans_probs_j, goal_state=absorb_goal_state)
    ...
    def per_traj(h_TH):
        return jax.vmap(per_t)(h_TH)

    return jax.vmap(per_traj)(hs)
    ...
    *,
    ...
    for it in range(iters):
        ...
        logemit_train = build_logemit_list(
            R_state, train_hoh, trans_probs_j,
            discount=vi_discount, vi_iters=vi_iters, tau=tau,
            absorb_goal_state=absorb_goal_state
        )
...
logemit_train = build_logemit_list(
    new_R_state, train_hoh, trans_probs_j,
    discount=0.95, vi_iters=50, tau=1.0,
    absorb_goal_state=ABSORB_GOAL_STATE
)
...
# test acc + loglik
logemit_test = build_logemit_list(
    new_R_state, test_hoh, trans_probs_j,
    discount=0.95, vi_iters=50, tau=1.0,
    absorb_goal_state=ABSORB_GOAL_STATE
)
...
# 1) Oracle policy
# IMPORTANT:
# For the current SoftVI-generated GW5 datasets, the correct oracle is the policy
# induced by the actual ground-truth reward maps RG_sa.npy, not the old handcrafted
# heuristic oracle build_true_behavior_oracle_gw5(...).
pi_oracle = build_oracle_policy_from_RG(
    trans_prob,
    RG,
    discount=0.95,
    vi_iters=50,
...
# IMPORTANT:
# Use the same evaluation tau as training/logemit reconstruction for internal
# consistency. The previous /0.5 sharpened the policy artificially.
def soft_vi_wrapper(R_sa_k, trans_prob_):
    tp_plan = np.array(make_goal_absorbing(jnp.array(trans_prob_), goal_state=ABSORB_GOAL_STATE))
    return np.array(soft_vi_sa(tp_plan, np.asarray(R_sa_k) / 1.0, discount=0.95, threshold=50))
