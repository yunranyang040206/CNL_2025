...
def _safe_normalize(v, axis=-1, eps=1e-12):
    return v / jnp.clip(jnp.sum(v, axis=axis, keepdims=True), a_min=eps)

def _mix_with_uniform(p, alpha, axis=-1):
    """
    Entropy-floor style smoothing:
...
def _log_from_probs(p, eps=1e-12):
...
# Student / semi-amortized inference code removed in this teacher-only cleanup.
