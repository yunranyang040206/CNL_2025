...
Example usage:
  python gtrxl_train_gw5.py --xs_path ../data_new/xs_fixed.npy --out_dir ../output/gtrxl_newset1 
  --epochs 40 --chunk_len 96 --H 5 --samples_per_traj 120 --batch_size 128 
  --lr 5e-4 --weight_decay 1e-2 --dropout 0.2 --d_model 32 --d_ff 64 --n_layers 1 --lambda_occ 0.4 --device cuda
"""
...
def main():
    ...
    ap.add_argument(
        ...
        nargs="*",
        default=[10,100],
        help="Past-window sizes W for true windowed export"
    )
    ...
    # Full-prefix export (current behavior, kept for comparison)
    ...
    print(f"[ok] Full-prefix embeddings exported to {out_embed} (key='h')")

    # True past-window exports
    export_embeddings_windowed(
        ckpt_path=str(best_path),
        xs_path=str(xs_npz),
        out_dir=str(out_dir),
        ...
        n_heads=args.n_heads,
        d_ff=args.d_ff,
        mem_len=0,  # must be 0 for true finite-window export
        gate_bias_init=args.gate_bias_init,
        batch_size=64,
        device=str(device),
    )

    print(f"[ok] Windowed embeddings exported for W in {args.window_sizes}")
