...
def best_perm_corr_KSA(R_pred: np.ndarray, R_gt: np.ndarray, mask_SA: np.ndarray):
    ...
    for perm in permutations(range(K)):
        ...
        per_mode_corr = []
        calib_params = []
        # Calibrate per mode before computing correlation.
        for k in range(K):
...
def _valid_action_mask(trans_prob: np.ndarray) -> np.ndarray:
    ...
    Why:
    - The oracle explore policy is uniform over valid actions.
    - KL/cross-entropy should exclude invalid actions.
    """
...
def _normalize_policy(pi: np.ndarray, valid: Optional[np.ndarray] = None) -> np.ndarray:
    ...
    Why:
    - When building oracle policies or reconstructing learned policies,
...
def learned_policy_from_R_avg(
    ...
    - R_avg summarizes the history-conditioned reward outputs into a stable table.
...
def build_oracle_policy_from_RG(
    ...
    for k in range(K):
        # soft_vi_sa expects (trans_prob, reward_sa).
...
def summarize_time_windowed(R_pred: np.ndarray, gamma: np.ndarray, T: int,
    ...
    """
    Convenience wrapper returning the core objects for printing or plotting.
