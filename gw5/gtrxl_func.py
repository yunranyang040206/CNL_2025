...
@torch.no_grad()
def export_embeddings_windowed(
    ckpt_path: str,
    xs_path: str,
    out_dir: str,
    ...
    """
    Compute TRUE past-window embeddings for all trajectories and save one file per window size.

    For each time t and window W, we feed only xs[:, max(0,t-W+1):t+1] into the causal encoder,
    then take the LAST token embedding as h_t^(W).

    Saves:
      out_dir / f"h_gw5_w{W}.npz"  with key 'h' and shape (N, T, d_model)
    """
    xs = load_xs(xs_path)
    N, T = xs.shape
    ...
    encoder = GTrXLEncoder(
        ...
        d_ff=d_ff,
        dropout=0.0,
        mem_len=0,  # IMPORTANT: true finite window, no cross-window memory
        gate_bias_init=gate_bias_init,
    ).to(dev)
    encoder.load_state_dict(enc_state)
    encoder.eval()
    ...
    # Pre-convert once
    xs_torch = torch.from_numpy(xs).to(dev)
    ...
    for W in window_sizes:
        ...
        # Export one timestep at a time, batched over trajectories
        for t in range(T):
            s = max(0, t - W + 1)
            x_win = xs_torch[:, s:t+1]   # (N, Lw), where Lw <= W

            for i0 in range(0, N, batch_size):
                i1 = min(N, i0 + batch_size)
                h_win, _ = encoder(x_win[i0:i1], mems=None, start_pos=0)
                # take the LAST token embedding = windowed h_t
                h_last = h_win[:, -1, :]  # (B, d_model)
                h_all[i0:i1, t, :] = h_last.detach().cpu().numpy().astype(np.float32)

        out_path = out_dir_p / f"h_gw5_w{W}.npz"
        np.savez_compressed(out_path, h=h_all)
        print(f"[export-windowed] W={W} | xs: {xs.shape} -> h: {h_all.shape} saved to {out_path}")