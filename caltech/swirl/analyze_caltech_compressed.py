"""
Analysis script for the Caltech SWIRL experiment on COMPRESSED sequences.

Loads pre-trained compressed net1 / net2 results, prints a comparison table,
and saves plots to ../results/ with 'compressed_' prefix.
...
# ── load results ──────────────────────────────────────────────────────────────
net1 = np.load(save_folder + f'{K}_{seed}_NM_caltech_compressed_net12.npz', allow_pickle=True)
net2 = np.load(save_folder + f'{K}_{seed}_NM_caltech_compressed_net22.npz', allow_pickle=True)
...
for k in range(K):
    print(f"  h{k+1}: {float(np.mean(zs1 == k)):.3f}")

print(f"\n{'State occupancy (net2)':}")
for k in range(K):
    print(f"  h{k+1}: {float(np.mean(zs2 == k)):.3f}")
...
plt.tight_layout()
path = save_folder + f'compressed_{K}_{seed}_training_curves2.pdf'
...
for ax, zs, label in [(axes[0], zs1, 'net1 (S-1)'), (axes[1], zs2, 'net2 (S-2)')]:
    ...
    ax.grid(False)
plt.tight_layout()
path = save_folder + f'compressed_{K}_{seed}_segments2.svg'
plt.savefig(path, bbox_inches='tight')
plt.close()
print(f"Saved {path}")
...
def _load_multi_K(model_tag, seed_):
    pattern = save_folder + f'*_{seed_}_NM_caltech_compressed_{model_tag}2.npz'
...
for model_tag, model_label in [('net1', 'S-1'), ('net2', 'S-2')]:
    ...
    plt.tight_layout()
    path = save_folder + f'compressed_multiK_{seed}_{model_tag}_comparison2.pdf'
    plt.savefig(path, bbox_inches='tight')
    plt.close()
    print(f"Saved {path}")
