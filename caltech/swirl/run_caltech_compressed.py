"""
Run SWIRL on the Caltech Mouse Social Behavior dataset using COMPRESSED sequences.

Compressed sequences have consecutive self-transitions removed, so only genuine
behavior switches remain.  File names are suffixed with 'compressed' throughout
to avoid interfering with the standard (non-compressed) pipeline.

Reads from ../data/:
  compressed_seqs.npy
  compressed_trans_probs.npy
  K_seed_arhmm_caltech_compressed.npz

Writes to ../results/:
  K_seed_NM_caltech_compressed_net1.npz
  K_seed_NM_caltech_compressed_net2.npz
...
seqs        = np.load(folder + 'compressed_seqs2.npy')
trans_probs = np.load(folder + 'compressed_trans_probs2.npy')
...
print(f"Trajectories: {trajs.shape[0]}  (compressed length {T_traj} each)")
...
arhmm_params  = np.load(folder + f'{K}_{seed}_arhmm_caltech_compressed.npz', allow_pickle=True)
...
jnp.savez(save_folder + f'{K}_{seed}_NM_caltech_compressed_net22.npz',
          new_logpi0=lp2, new_log_Ps=lP2, new_Rs=R2,
          new_R_state=Rs2.params, train_LL_list=LL2_train, test_LL_list=LL2_test)
print(f"Saved {K}_{seed}_NM_caltech_compressed_net22.npz\n")
...
jnp.savez(save_folder + f'{K}_{seed}_NM_caltech_compressed_net12.npz',
...
print(f"Saved {K}_{seed}_NM_caltech_compressed_net12.npz")
